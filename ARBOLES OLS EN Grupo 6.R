...
stargazer(as.data.frame(Bogota_train), type="text", title= "Estadísticas",
...
##ARBOLES---------------------------------------------------------------------
colnames(train_muestra) Superficie
...
arbol_1<-train(price ~ bedrooms + bathrooms + lat + lon + Superficie+  property_type + distancia_parque+ distancia_comercial+ distancia_avenida_principal +
...
-------------------------------------------------------------------
###SUPER LEARNER
train_muestra<- train_muestra  %>% 
  mutate(logprice=log(price)) 
 
p_load("SuperLearner")

listWrappers()
ySL<-train_muestra$price
XSL<- train_muestra  %>% select(property_type, Superficie,
                                distancia_parque, distancia_comercial, distancia_avenida_principal)


sl.lib <- c("SL.randomForest", "SL.glmnet", "SL.ridge", "SL.lm" )
